...
void put(char * filename){
   ...
   // printf("%li\n", k);
    ...
    int sendingfile;
    sendingfile = hash(filename);
    //printf("%d\n", sendingfile); //returned a 0
    if (sendingfile == 0){
        send(, fp_piece1, strlen(fp_piece1), 0);
        send(, fp_piece1, strlen(fp_piece1), 0);

    }
    else if (sendingfile == 1){

    }
    else if(sendingfile == 2){


    }
    else if(sendingfile == 3){
...
void connectport(const char *portnumber){
...
int main(int argc, char *argv[]){
    ...
    connectport(configcount.dfsOne);
    connectport(configcount.dfsTwo);
    connectport(configcount.dfsThree);
    connectport(configcount.dfsFour);
